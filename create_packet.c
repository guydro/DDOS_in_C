...
#define dst_port 2869

#define dst_ip "192.168.0.2"
#define src_ip "192.168.0.5"

#define dst_mac "\x10\xda\x43\xdf\x31\xdc"
#define src_mac "\x7c\xc2\xc6\x31\x02\xf1"

#define ip_size (sizeof(struct ip_hdr) + sizeof(struct tcp_header) + payload_size)
#define total_size (sizeof(dst_mac) + sizeof(src_mac) + ip_size)
...
char* create_packet() {
    char* buf = calloc(total_size, 1); // buf contains the complete packet
    buf[total_size - 1] = 1; // Canary
    ...
    v4hdr->ip_tos = 0;
    v4hdr->ip_total_length = htons(ip_size);
    v4hdr->ip_id = htons(2);
    ...
    static unsigned int sequence = 0xABCDEFAA;
    sequence++;
    tcphdr->sequence = sequence;
    ...
    // You may add code to change buf[total_packet_size - payload_size] if you want to send a specific payload.
